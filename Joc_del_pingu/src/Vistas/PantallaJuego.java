...
import Controladores.GestorJugador;
...
public class PantallaJuego {
    ...
    private void handleDadoNormal(ActionEvent event) {
        ...
        Casilla casilla = gestorPartida.getPartida().getTablero().getCasilla(pos);

        comprobarCombateEnCasilla(p, pos, casilla);
    }
    ...
    private void comprobarCombateEnCasilla(Pinguino jugadorMovido, int posicion, Casilla casilla) {
        ...
        for (Jugador otro : jugadores) {
            if (otro == jugadorMovido || otro instanceof Foca) continue;
            if (!(otro instanceof Pinguino)) continue;

            if (otro.getPosicion() == posicion) {
                Pinguino otroPinguino = (Pinguino) otro;
                boolean esCasillaEspecial = (casilla instanceof Oso)
                        || (casilla instanceof Trineo)
                        || (casilla instanceof Agujero)
                        || (casilla instanceof Interrogante)
                        || (casilla instanceof SueloQuebradizo);

                if (esCasillaEspecial) {
                    mostrarDialogCombatOEfecte(jugadorMovido, otroPinguino);
                } else {
                    afegirMissatge("⚔️ " + jugadorMovido.getNombre()
                            + " i " + otroPinguino.getNombre()
                            + " es troben! Guerra de boles de neu automàtica!");
    ...
    private void mostrarDialogCombatOEfecte(Pinguino jugador, Pinguino rival) {
        javafx.scene.control.Alert alert = new javafx.scene.control.Alert(
                javafx.scene.control.Alert.AlertType.CONFIRMATION
        );
        alert.setTitle("Casella especial compartida!");
        alert.setHeaderText("Tant " + jugador.getNombre()
                + " com " + rival.getNombre() + " estan a la mateixa casella especial.");
        alert.setContentText("Què vols fer?");

        javafx.scene.control.ButtonType opcioEfecte = new javafx.scene.control.ButtonType(
                "Rebre l'efecte de la casella"
        );
        javafx.scene.control.ButtonType opcioGuerra = new javafx.scene.control.ButtonType(
                "⚔️ Guerra de boles de neu!"
        );

        alert.getButtonTypes().setAll(opcioEfecte, opcioGuerra);

        alert.showAndWait().ifPresent(resposta -> {
            if (resposta == opcioGuerra) {
                afegirMissatge("⚔️ " + jugador.getNombre()
                        + " tria lluitar contra " + rival.getNombre() + "!");
                executarGuerra(jugador, rival);
            } else {
                afegirMissatge("🏠 " + jugador.getNombre()
                        + " tria rebre l'efecte de la casella especial.");
            }
        });
    }
    ...
    private void executarGuerra(Pinguino p1, Pinguino p2) {
        ...
        afegirMissatge(p1.getNombre() + " té " + bolas1 + " boles ❄️ | "
                + p2.getNombre() + " té " + bolas2 + " boles ❄️");

        GestorJugador gestorJugadorLocal = new GestorJugador();
        gestorJugadorLocal.pinguinoGuerra(p1, p2);

        if (bolas1 > bolas2) {
            afegirMissatge("🏆 " + p1.getNombre() + " guanya! "
                    + p2.getNombre() + " retrocedeix " + (bolas1 - bolas2) + " caselles.");
        } else if (bolas2 > bolas1) {
            afegirMissatge("🏆 " + p2.getNombre() + " guanya! "
                    + p1.getNombre() + " retrocedeix " + (bolas2 - bolas1) + " caselles.");
        } else {
            afegirMissatge("🤝 Empat! Cap jugador retrocedeix. Tots perden les boles.");
        }
