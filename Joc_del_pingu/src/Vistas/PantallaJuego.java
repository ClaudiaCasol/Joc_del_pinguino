...
public class PantallaJuego {
    ...
    private void generarTableroVisual() {
        ...
        casillasVista.clear();
        ArrayList casillas = gestorPartida.getPartida().getTablero().getCasillas();
        for (int i = 0; i < casillas.size(); i++) {
            StackPane celda = crearCeldaVisual((Casilla) casillas.get(i), i);
    ...
    private StackPane crearCeldaVisual(Casilla casilla, int indice) {
        ...
        ImageView img = null;
        if (casilla instanceof Oso)          img = cargarImagen("/imatges/oso.png", 40);
        if (casilla instanceof Trineo)       img = cargarImagen("/imatges/trineo.png", 40);
        if (casilla instanceof Interrogante) img = cargarImagen("/imatges/interrogante.png", 35);
        if (casilla instanceof SueloQuebradizo && indice == sueloQuebradizoActivo)
                                             img = cargarImagen("/imatges/hielo-roto.png", 40);
        if (img != null) celda.getChildren().add(img);
    ...
    private void crearFichas() {
        ...
        for (int i = 0; i < jugadores.size(); i++) {
            ...
            String ruta;
            switch (i) {
                case 0:  ruta = "/imatges/skipper.jpg";   break;
                case 1:  ruta = "/imatges/kowalski.jpg";  break;
                case 2:  ruta = "/imatges/rico.jpg";      break;
                case 3:  ruta = "/imatges/private.jpg";   break;
                default: ruta = "/imatges/focaPingu.jpg"; break;
            }
    ...
    private void actualizarPosicionesVisuales() {
        for (Object obj : casillasVista) {
            ((StackPane) obj).getChildren().removeIf(n -> n instanceof Circle);
        }
        ...
        for (int i = 0; i < jugadores.size(); i++) {
            ...
            Circle ficha = (Circle) fichasVista.get(i);
            ...
            if (j.getPosicion() < 0) {
                ...
                StackPane celda = (StackPane) casillasVista.get(j.getPosicion());
                ficha.setTranslateX(0);
                ficha.setTranslateY(0);
                StackPane.setAlignment(ficha, Pos.CENTER);
                double[][] posiciones = { {-10, -10}, {10, -10}, {-10, 10}, {10, 10} };
    ...
    private void handleDadoNormal(ActionEvent event) {
        ...
        if (!(jugadorActual instanceof Pinguino)) {
            afegirMissatge("És el torn de la foca CPU...");
            return;
        }
        ...
        Casilla casilla = (Casilla) gestorPartida.getPartida().getTablero().getCasillas().get(pos);

        PauseTransition pausa = new PauseTransition(Duration.seconds(1.2));
        pausa.setOnFinished(e -> {
            if      (casilla instanceof Oso)             { System.out.println("SONIDO OSO");    audio.reproducirEfecto("/audio/oso.mp3"); }
            else if (casilla instanceof Agujero)         { System.out.println("SONIDO agujero"); audio.reproducirEfecto("/audio/agujero.mp3"); }
            else if (casilla instanceof Trineo)          { System.out.println("SONIDO trineo");  audio.reproducirEfecto("/audio/trineo.mp3"); }
            else if (casilla instanceof SueloQuebradizo) { System.out.println("SONIDO suelo");   audio.reproducirEfecto("/audio/sueloQuebradizo.mp3"); }
            else if (casilla instanceof Interrogante)    { System.out.println("SONIDO suelo");   audio.reproducirEfecto("/audio/exclamacion.mp3"); }
        });
        pausa.play();
    }
    ...
    private void ejecutarUnTurno() {
        ...
        if (despues >= 0 && despues < partida.getTablero().getCasillas().size()) {
            Casilla c = (Casilla) partida.getTablero().getCasillas().get(despues);
            if (c instanceof SueloQuebradizo) sueloQuebradizoActivo = despues;
        }
        ...
        if (despues >= 0 && despues < casillasVista.size()) {
            animarCasillaDestino((StackPane) casillasVista.get(despues));
        }
    ...
    private Dado buscarDadoSegunTipo(Jugador jugador, String tipo) {
        if (!(jugador instanceof Pinguino)) return null;
        Pinguino p = (Pinguino) jugador;
        if (p.getInventario() == null) return null;

        ArrayList dados = p.getInventario().getDado();
        for (Object obj : dados) {
            Dado d = (Dado) obj;
            if (tipo.equalsIgnoreCase("rapido") && d instanceof Dado_rapido) return d;
            if (tipo.equalsIgnoreCase("lento")  && d instanceof Dado_lento)  return d;
            if (tipo.equalsIgnoreCase("normal")
                    && !(d instanceof Dado_rapido)
                    && !(d instanceof Dado_lento)) return d;
        }
        for (Object obj : dados) {
            Dado d = (Dado) obj;
            if (!(d instanceof Dado_rapido) && !(d instanceof Dado_lento)) return d;
        }
        return null;
    }

    private void consumirDadoEspecialSiHaceFalta(Jugador jugador, Dado dadoUsado) {
        if (!(jugador instanceof Pinguino) || dadoUsado == null) return;
        Pinguino p = (Pinguino) jugador;
        if (!(dadoUsado instanceof Dado_rapido) && !(dadoUsado instanceof Dado_lento)) return;
    ...
    private void afegirMissatge(String text) {
        if (eventos        != null) eventos.setText(text);
        if (consolaEventos != null) consolaEventos.appendText(text + "\n");
    }
    ...
    private void actualizarTooltipInventario() {
        ...
        int dadosNormales = 0, dadosRapidos = 0, dadosLentos = 0;
        for (Object obj : inv.getDado()) {
            Dado d = (Dado) obj;
            if      (d instanceof Dado_rapido) dadosRapidos++;
            else if (d instanceof Dado_lento)  dadosLentos++;
            else                               dadosNormales++;
        }
